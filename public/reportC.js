document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("downloadPDFC").addEventListener("click", async () => {
        try {
            ...
            // 🔹 System Metrics Section (Server C)
            pdf.setFontSize(22);
            pdf.setFont("helvetica", "bold");
            pdf.setTextColor(0, 102, 204);
            pdf.text("Server C Metrics", 10, 20);

            const serverCMetrics = [
                ["Metric", "Value"],
                ["EC2 Instance Status", document.getElementById("ec2-statusC")?.innerText || "N/A"],
                ["Docker Status", document.getElementById("docker-statusC")?.innerText || "N/A"],
                ["Docker Refresh Time", document.getElementById("docker-refresh-timeC")?.innerText || "N/A"],
                ["CPU Usage", document.getElementById("cpu-usageC")?.innerText || "N/A"],
                ["CPU Refresh Time", document.getElementById("cpu-refresh-timeC")?.innerText || "N/A"],
                ["RAM Usage", document.getElementById("ram-usageC")?.innerText || "N/A"],
                ["RAM Refresh Time", document.getElementById("ram-refresh-timeC")?.innerText || "N/A"],
                ["Root FS Usage", document.getElementById("root-fs-usageC")?.innerText || "N/A"],
                ["Root FS Refresh Time", document.getElementById("root-fs-refresh-timeC")?.innerText || "N/A"],
                ["Server Uptime", document.getElementById("server-uptimeC")?.innerText || "N/A"],
                ["Uptime Refresh Time", document.getElementById("uptime-refresh-timeC")?.innerText || "N/A"],
            ];

            pdf.autoTable({
                startY: 30,
                head: [serverCMetrics[0]],
                body: serverCMetrics.slice(1),
                theme: "grid",
            ...
            // 🔹 Chart Section (Server C)
            const charts = [
                { id: "requestsErrorsChartC", title: "Requests & Errors Over Time (Server C)" },
                { id: "networkTrafficChartC", title: "Network Traffic Over Time (Server C)" },
            ];
            ...
            for (const chart of charts) {
                ...
                if (chartElement) {
                    ...
                    if (chart.id === "networkTrafficChartC") {
            ...
            const gptInsights = await generateGPTInsights(serverCMetrics);
            ...
            // Save the PDF
            pdf.save("Lumiere_Proxy_ServerC_Report.pdf");
