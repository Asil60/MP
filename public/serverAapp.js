...
async function fetchServerUptime() {
  ...
  async function fetchAndUpdateChart() {
    try {
        ...
        if (!ctx) {
            console.error("Canvas element 'requestsErrorsChartA' not found.");
            return;
        }
...
async function fetchAndUpdateNetworkTrafficChart() {
    try {
        ...
        if (!ctx) {
            console.error("Canvas element 'networkTrafficChartA' not found.");
            return;
        }
...
async function fetchSummary() {
    try {
        ...
        const data = {
            nginxStatus: document.getElementById("nginx-statusA").textContent || "N/A",
            ...
            networkTrafficChart: document.getElementById("networkTrafficChartA").textContent || "N/A",
        };
...
async function fetchNodeJsStatus() {
    try {
      const response = await fetch("/nodejs-statusA");
      const data = await response.json();
 
      if (data.status === undefined) throw new Error("No Nodejs status data available.");
 
      const nodejsStatus = data.status === 1 ? "ON" : "OFF";
      document.getElementById("nodejs-statusA").textContent = nodejsStatus;
 
      const now = new Date();
      document.getElementById("nodejs-refresh-timeA").textContent = `Last updated at ${now.toLocaleTimeString()}`;
    } catch (error) {
      console.error("Error fetching Nodejs status:", error);
      document.getElementById("nodejs-statusA").textContent = "Error fetching data.";
      document.getElementById("nodejs-refresh-timeA").textContent = "";
    }
  }
