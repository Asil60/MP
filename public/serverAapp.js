...
// ✅ Function to fetch and update RAM usage based on selected timeframe for Server A
async function fetchRAMUsageARange(start) {
    try {
        ...
        const now = new Date();
        if (isLiveModeActiveA) {
            document.getElementById("ram-refresh-timeA").textContent = `Last updated at: ${now.toLocaleString()}`;
        } else {
            document.getElementById("ram-refresh-timeA").textContent = `Data fetched for range starting at: ${new Date(start * 1000).toLocaleString()}`;
        }
    } catch (error) {
        console.error("Error fetching RAM usage data:", error);
        document.getElementById("ram-usageA").textContent = "Error fetching data.";
...
// ✅ Function to fetch and update Root FS usage based on selected timeframe for Server A
async function fetchRootFSUsageARange(start) {
    try {
        ...
        // ✅ Display the latest and average Root FS usage
        if (latestValue !== null) {
            document.getElementById("root-fs-usageA").textContent = `${latestValue.toFixed(1)}%`;
        } else {
            document.getElementById("root-fs-usageA").textContent = "N/A";
        }
        ...
        const now = new Date();
        if (isLiveModeActiveA) {
            document.getElementById("root-fs-refresh-timeA").textContent = `Last updated at: ${now.toLocaleString()}`;
        } else {
            document.getElementById("root-fs-refresh-timeA").textContent = `Data fetched for range starting at: ${new Date(start * 1000).toLocaleString()}`;
        }
    } catch (error) {
        console.error("Error fetching Root FS usage data:", error);
        document.getElementById("root-fs-usageA").textContent = "Error fetching data.";
...
async function fetchServerASummary() {
    try {
        ...
        // ✅ Get All Network Traffic Data from Chart.js
        console.log(Chart.getChart("networkTrafficChartA"));
        const chartInstance = Chart.getChart("networkTrafficChartA"); // Get Chart.js instance
        if (chartInstance) {
